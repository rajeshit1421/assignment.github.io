...
abstract class REST_Controller extends CI_Controller {
	...
	Custom method
	*/
	function checkValidUser($table, $user_id)
	{
        $this->load->model('Api_model');
		$userData = $this->Api_model->getRecord($table, array('admin_id' => $user_id,'status'=>1,'(delete_status = 0 OR delete_status IS NULL)'=>NULL));
        #print_r($userData);die;
        $response = array();
		if (is_object($userData) == TRUE) {
            $response['status'] = true;
            $response['userData'] = $userData;
            return $response;
		} else {
            $this->response(['returnstatus' => REST_Controller::HTTP_UNAUTHORIZED, 'message' => 'User not found!']);
		}
    }
}
